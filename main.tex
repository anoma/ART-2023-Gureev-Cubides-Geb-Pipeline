...
The Geb project was initiated to compile generic functional programs into
...
The core of this approach rests on the observation that effective
...
representing those programming languages.  A canonical example is that of Lambek-Scott, dealing with the fact that the languages possessing STLC structure form a category which is equivalent to the category of cartesian closed categories.
...
With this, we are done. One may notice that we actually skipped the \lamberr term, yet that is because what we described is the compilation of a term without errors. We have a predicate checking whether there is or is not an error term in a given term which decided how to compile it further. 

The compilation for terms with \lamberr is similar to the procedure described above, but wrapping everything recursively into a \tcbox{Maybe} monad.
