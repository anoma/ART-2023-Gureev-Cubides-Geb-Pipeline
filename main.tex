...
\begin{keywords}
Geb; Juvix; VampIR; compilers; category theory; semantics of PL; Lambda
calculus;
\end{keywords}
\maketitle
...
that is accessible to people without delving into specific implementation.
Primarily, we aim to:
...
category of the theory of categories with finite coproduct on one object.
Given that $C$ has coproducts and $1_C$, we can establish the functor $F
...
\tcbox{parallel-seq} is actually the product of the corresponding
...
\begin{remark}
    ...
    \tcbox{lambda} package. Below we showcase the compilation of the
    identity function.
...
arithmetic functions of form \tcbox{nat-add n : (nat-width n) $\times$
(nat-width n) $\to$ (nat-width n)} which we want to be a \VampIR{} function
...
\end{minted}
 ...
 $(a_1,\dots,a_n), (b_1,\dots,b_m), (c_1,\dots,c_k)$ accordingly. Proceed
 by induction on \tcbox{t : substmorph}.
...
\begin{table}[!htb]
\label{table:compilation-to-cat-part-2}
\caption{Continuation for \Cref{table:compilation-to-cat-part-1} on
compiling from \Geb{} to \SeqN{}.} 
...
\begin{minted}[style=borland]{python}
def name x1 ... xn = { range a1 x1 ... range an xn X }
\end{minted}
