...
specified in~\cite{LS-catlog}, dealing with the fact that the
...
\tcbox{to-seqn}. The same applies to \tcbox{to-circuit}.
...
\hspace*{2mm}(coprod \\
\hspace*{2.5mm}(prod a b) \\
\hspace*{2.5mm}(prod a c)))
\end{tabular}
}\hfill
% \small\tcbox[tikznode]{%
%  distribute : \\
...
\begin{longtable}{llp{\q}}
    ...
    \end{tabular}} &      \texttt{x1,...,x(n+m)}  
	& 
	\texttt{(f1,...,fn,g1,...,gm)}\footnote{Inductive hypotheses: {\tcbox[tikznode]{to-vampir (f) (x1,...,xn) = (f1,....,fn)}} and {\tcbox[tikznode]{to-vampir (g) (x(n+1),...,x(n+m))= (g1,....,gm)}}.}
	\\
	\midrule
	\small\tcbox{%
    \begin{tabular}{l}
